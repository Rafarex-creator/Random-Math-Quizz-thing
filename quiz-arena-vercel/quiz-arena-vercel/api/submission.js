...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    const quiz = await redis.get(`quiz:${c}`);
    ...
    await redis.set(`submission:${c}:${participantId}`, sub);
  ...
  if (req.method === 'GET') {
    ...
    if (participantId) {
      const sub = await redis.get(`submission:${c}:${participantId}`);
      if (!sub) return res.status(404).json({ error: 'Result not found.' });
      return res.status(200).json(sub);
    }

    const keys = await redis.keys(`submission:${c}:*`);
    const subs = [];
    for (const k of keys) {
